...
2. It must be between 2 and 4 It will be changed after each plan changes. The initial value of threshold for each plan must be 2. Let's say a new plan has been to the customer the threshold must be set to 2.
...
4. Let,s segregate the customer sentiment into 4 categories:
  **1. Positive : Willing to move with the current plan and asking for information abput current plan. Assign a value between 2 to 3 to threshold.
  **2 Neagitive: He want to move to the next plan and dont want to proceed with the current plan. Assign a value of 1 to threshold.
  **3 Unsure: When Customer doesnt understand the plan try to explain the current plan to the customer.Assign a value between 1 to 2  threshold.
